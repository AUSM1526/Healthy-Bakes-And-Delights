...
const DeleteSubCategoryModal = ({ isOpen, onClose, subCategoryId , subCategoryName, onSuccess }) => {
  const [loading, setLoading] = useState(false);
  
  const handleDelete = async() => {
    setLoading(true);
    try {
        await apiFunc().delete(`/subCategory/${subCategoryId}`);
        ...
        {!loading && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <div className="bg-chocolate-dark rounded-2xl p-6 w-[90%] max-w-md shadow-2xl border border-[#d8ccc1] dark:border-[#333] relative transition-all">
                    ...
                    Are you sure you want to delete <span className="font-semibold">{subCategoryName}</span>? This action cannot be undone.
                    </p>
